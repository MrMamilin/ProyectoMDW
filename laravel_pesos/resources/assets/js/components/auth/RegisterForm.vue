<template>
 <form method="post" @submit.prevent="submit" @keydown="clearErrors($event.target.name)">
  ...
  <div class="form-group has-feedback" :class="{ 'has-error': form.errors.has('address') }">
    <input type="text" class="form-control" :placeholder="trans('adminlte_lang_message.address')" name="address" v-model="form.address"/>
    <span class="glyphicon glyphicon-home form-control-feedback"></span>
    <transition name="fade">
        <span class="help-block" v-if="form.errors.has('address')" v-text="form.errors.get('address')"></span>
    </transition>
</div>
...
export default {
  ...
  data: function () {
    return {
      form: new Form({ name: '', email: '', password: '', password_confirmation: '', address: '',})
    }
  },
  watch: {
